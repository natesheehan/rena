...
ena_download <- function(download_type,
                         query = NULL,
                         accession = NULL,
                         format, location = getwd(),
                         email = "NONE") {
  # Semantic validation of variables
  download_type <- tolower(download_type)
  valid_types <- c("q", "a")
  valid_formats <- c("READS_FASTQ", "READS_SUBMITTED", "ANALYSIS_SUBMITTED", "ANALYSIS_GENERATED")

  if (!download_type %in% valid_types) {
    stop("Wrong download type has been given. Please use 'q' for query or 'a' for accession.")
  }
  if (!format %in% valid_formats) {
    stop("Wrong format type has been given. Please use one of: READS_FASTQ, READS_SUBMITTED, ANALYSIS_SUBMITTED, ANALYSIS_GENERATED.")
  }
